...
export default function MyProfile() {
      ...
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-green-50 flex items-center justify-center">
        <div className="text-center">
          <div className="relative">
            <div className="animate-spin rounded-full h-32 w-32 border-4 border-green-200 border-t-emerald-500 mx-auto mb-6"></div>
  ...
  return (
    <div className="w-full px-4 md:px-6 py-4 md:py-8 bg-gradient-to-br from-emerald-50 to-green-50 min-h-screen">
      {/* Toast Notification */}
      {toast.show && (
      ...
      <div className="w-full px-4 md:px-8 py-6 md:py-10 min-h-screen">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 mb-6 md:mb-10">
          <div className="bg-gradient-to-br from-emerald-50 to-green-100 border border-emerald-200 text-emerald-800 rounded-xl md:rounded-2xl p-3 md:p-6 shadow-sm hover:shadow-md transition-all duration-300">
            <div className="flex items-center justify-between">
              ...
              <div className="text-2xl md:text-4xl text-emerald-600">⏰</div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-teal-50 to-teal-100 border border-teal-200 text-teal-800 rounded-xl md:rounded-2xl p-3 md:p-6 shadow-sm hover:shadow-md transition-all duration-300">
            <div className="flex items-center justify-between">
              ...
              <div className="text-2xl md:text-4xl text-teal-600">💰</div>
            </div>
          </div>
        ...
        <div className="bg-gradient-to-br from-emerald-50 to-white rounded-xl md:rounded-2xl shadow-sm border border-emerald-200 p-4 md:p-6 mb-6 md:mb-10">
          <div className="flex flex-col md:flex-row items-center gap-4 md:gap-8">
            ...
            <div className="cursor-pointer hover:scale-105 transition-all duration-300">
              {photo ? (
                <img
                  src={photo}
                  alt="Foto"
                  className="w-24 h-24 md:w-32 md:h-32 rounded-full object-cover border-4 border-emerald-200 shadow-lg"
                />
              ) : (
                <div
                  className="w-24 h-24 md:w-32 md:h-32 rounded-full flex items-center justify-center text-2xl md:text-4xl font-extrabold shadow-lg border-4 border-emerald-200"
            ...
            <div className="flex-1 text-center md:text-left">
              <h2 className="text-2xl md:text-4xl font-extrabold text-emerald-800 mb-3 md:mb-4">
              ...
              <div className="flex flex-wrap gap-2 md:gap-4 mb-4 md:mb-8 justify-center md:justify-start">
                ...
                <span className="text-xs font-semibold text-white bg-gradient-to-r from-emerald-500 to-green-500 px-2 md:px-3 py-1 rounded-full shadow-sm uppercase tracking-wide">{role}</span>
                <span className="text-xs font-semibold text-emerald-800 bg-emerald-100 px-2 md:px-3 py-1 rounded-full border border-emerald-200">{label_type}</span>
                <span className="text-xs font-semibold text-green-800 bg-green-100 px-2 md:px-3 py-1 rounded-full border border-green-200">{qualification}</span>
              </div>
        ...
        <div className="w-full bg-gradient-to-br from-emerald-50 to-white rounded-xl md:rounded-2xl shadow-sm border border-emerald-200 p-4 md:p-8 mb-6 md:mb-10">
          ...
          <Calendar
            ...
            className="border-2 border-emerald-200 rounded-xl md:rounded-2xl shadow-xl w-full text-sm md:text-lg"
          />
          <div className="flex gap-2 md:gap-4 mt-4 md:mt-6 flex-wrap justify-center">
            {employeeSites.map((site) => (
              <div key={site} className="flex items-center gap-2 bg-white/80 rounded-xl px-3 md:px-4 py-2 shadow-md border border-emerald-100">
                ...
                <span className="text-sm md:text-lg font-bold text-emerald-800">{site}</span>
              </div>
            ))}
          </div>
          ...
          {selectedDate && (
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-xl md:rounded-2xl shadow-2xl p-4 md:p-8 min-w-[280px] md:min-w-[320px] max-w-md relative animate-fade-in">
                ...
                <h4 className="text-lg md:text-xl font-bold mb-3 md:mb-4 text-emerald-700">Detajet për {selectedDate.toLocaleDateString()}</h4>
                {filteredWorkDays[selectedDate.toISOString().slice(0, 10)]?.length ? (
                  <ul className="space-y-2 md:space-y-3">
                    {filteredWorkDays[selectedDate.toISOString().slice(0, 10)].map((entry, idx) => (
                      <li key={idx} className="flex flex-col gap-1 bg-emerald-50 rounded-xl p-3 shadow">
        ...
        <div className="bg-gradient-to-br from-emerald-50 to-white rounded-xl md:rounded-2xl shadow-sm border border-emerald-200 p-4 md:p-6 mb-6 md:mb-10">
          ...
          <div className="mb-4 p-4 bg-emerald-50 rounded-xl border-2 border-dashed border-emerald-200">
            <input 
              ...
              className="w-full p-2 border-2 border-emerald-200 rounded-xl bg-white"
          ...
          {attachments.length > 0 ? (
            <ul className="space-y-2">
              {attachments.map(doc => (
                <li key={doc.id} className="flex items-center gap-4 bg-emerald-50 rounded-xl p-3 shadow">
                  <a
                    href={doc.file_path}
                    download={doc.file_name}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-emerald-700 underline font-semibold flex items-center gap-2"
                  >
          ...
          {tasks.length > 0 ? (
            <div className="grid gap-3 md:gap-4">
              {tasks.slice(0, 10).map((task, index) => (
                <div key={task.id || index} className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4 md:p-6 shadow-lg border border-green-200">
                  <h4 className="text-lg md:text-xl font-bold text-emerald-800 mb-2">{task.title}</h4>
                  <p className="text-gray-700 mb-3 text-sm md:text-base">{task.description}</p>
                  <div className="flex flex-wrap gap-2 md:gap-4 text-xs md:text-sm">
                    <span className="px-2 md:px-3 py-1 rounded-full text-xs font-bold border bg-emerald-100 text-emerald-800 border-emerald-200">
                      ✅ Përfunduar
                    </span>
        ...
        <div className="w-full bg-gradient-to-br from-emerald-50 to-white rounded-xl md:rounded-2xl shadow-sm border border-emerald-200 p-4 md:p-6 mb-6 md:mb-10">
          ...
          <div className="space-y-3 md:space-y-4">
            {Object.entries(workHistory).map(([weekLabel, days]) => {
              ...
              return (
                <div key={weekLabel} className="bg-gradient-to-r from-emerald-50 to-green-50 rounded-xl p-3 md:p-4 border border-emerald-200 shadow-sm">
                    ...
                    <div className="flex flex-col gap-1 ml-2 md:ml-4">
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="bg-gradient-to-r from-emerald-500 to-green-500 text-white px-3 md:px-4 py-2 rounded-xl font-bold shadow-sm hover:from-green-600 hover:to-emerald-600 transition-all duration-300 text-xs md:text-sm disabled:opacity-50"
                        >
                          {submittingNote === weekLabel ? '⏳' : '💾'}
                        </button>
                      </div>
                      {weekNotes[weekLabel] && (
                        <span className="text-xs text-emerald-700 italic">{weekNotes[weekLabel]}</span>
                      )}
                    </div>
                  </div>
                </div>
      ...
      {showPasswordModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-gradient-to-br from-emerald-50 to-white rounded-xl md:rounded-2xl shadow-sm border border-emerald-200 p-6 md:p-8 min-w-[320px] md:min-w-[400px] max-w-md relative animate-fade-in">
            <button 
              onClick={() => setShowPasswordModal(false)} 
              className="absolute top-4 right-4 text-xl md:text-2xl text-emerald-500 hover:text-emerald-700"
            >
              &times;
            </button>
            <h3 className="text-xl md:text-2xl font-bold text-emerald-800 mb-6">🔐 Ndrysho Fjalëkalimin</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-semibold text-emerald-700 mb-2">Fjalëkalimi Aktual</label>
                <input
                  type="password"
                  value={passwordData.current}
                  onChange={e => setPasswordData(prev => ({ ...prev, current: e.target.value }))}
                  className="w-full p-3 border-2 border-emerald-200 rounded-xl focus:ring-2 focus:ring-emerald-300 bg-white"
                  placeholder="Shkruaj fjalëkalimin aktual"
                />
              </div>
              <div>
                <label className="block text-sm font-semibold text-emerald-700 mb-2">Fjalëkalimi i Ri</label>
                <input
                  type="password"
              ...
              <div>
                <label className="block text-sm font-semibold text-emerald-700 mb-2">Konfirmo Fjalëkalimin e Ri</label>
                <input
                  type="password"
              ...
              <div className="flex gap-3 pt-4">
                <button
                  ...
                  className="flex-1 bg-gradient-to-r from-emerald-500 to-green-600 text-white px-4 py-3 rounded-xl font-bold shadow-sm hover:from-green-700 hover:to-emerald-700 transition-all duration-300"
                >
                  💾 Ruaj
                </button>
                <button
                  ...
                  className="flex-1 bg-gradient-to-r from-emerald-500 to-green-500 text-white px-4 py-3 rounded-xl font-bold shadow-sm hover:from-green-700 hover:to-emerald-700 transition-all duration-300"
                >
                  Anulo
                </button>
              </div>
