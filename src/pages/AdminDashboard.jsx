...
export default function AdminDashboard() {
  // State për dashboard stats
  const [dashboardStats, setDashboardStats] = useState(null);

  // Merr të gjitha statistikat nga backend me një API call
  useEffect(() => {
  ...
  // Rendero loading nëse nuk ka ardhur ende
  if (!dashboardStats) {
    return <div className="p-10 text-center text-xl text-blue-700">Duke ngarkuar statistikat...</div>;
  }
  ...
  return (
    <div className="w-full max-w-full px-2 md:px-8 py-6 md:py-10 min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      {/* Shembull: Cards statistikash nga totals */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-10">
        <div className="bg-gradient-to-br from-blue-100 to-blue-200 rounded-2xl shadow-lg p-6 flex flex-col items-center">
          ...
          <div className="text-2xl font-bold text-blue-800">£{dashboardStats.totals?.weekly?.paid?.toLocaleString()}</div>
          <div className="text-sm text-blue-700">Pagesa këtë javë</div>
        </div>
        <div className="bg-gradient-to-br from-green-100 to-green-200 rounded-2xl shadow-lg p-6 flex flex-col items-center">
          <div className="text-4xl mb-2">🕒</div>
          <div className="text-2xl font-bold text-green-800">{dashboardStats.detailedStats?.totalWorkHours?.weekly}</div>
          <div className="text-sm text-green-700">Orë pune këtë javë</div>
        </div>
        <div className="bg-gradient-to-br from-purple-100 to-purple-200 rounded-2xl shadow-lg p-6 flex flex-col items-center">
          <div className="text-4xl mb-2">📈</div>
          <div className="text-2xl font-bold text-purple-800">£{dashboardStats.totals?.weekly?.profit?.toLocaleString()}</div>
          <div className="text-sm text-purple-700">Fitim këtë javë</div>
        </div>
        <div className="bg-gradient-to-br from-yellow-100 to-yellow-200 rounded-2xl shadow-lg p-6 flex flex-col items-center">
          <div className="text-4xl mb-2">💸</div>
          <div className="text-2xl font-bold text-yellow-800">£{dashboardStats.totals?.weekly?.expenses?.toLocaleString()}</div>
          <div className="text-sm text-yellow-700">Shpenzime këtë javë</div>
        </div>
      </div>
