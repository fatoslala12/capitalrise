...
const NotificationBell = () => {
  ...
  const formatTimeAgo = (dateString) => {
    if (!dateString) return t('notifications.now', 'Now');
    ...
    if (isNaN(date.getTime())) {
      return t('notifications.now', 'Now');
    }
    
    const diffInMinutes = Math.floor((now - date) / (1000 * 60));
    console.log('[DEBUG] Diff in minutes:', diffInMinutes);
    
    if (diffInMinutes < 1) return t('notifications.now', 'Now');
    if (diffInMinutes < 60) return t('notifications.minutesAgo', '{{count}}m ago').replace('{{count}}', diffInMinutes);
    
    const diffInHours = Math.floor(diffInMinutes / 60);
    if (diffInHours < 24) return t('notifications.hoursAgo', '{{count}}h ago').replace('{{count}}', diffInHours);
    
    const diffInDays = Math.floor(diffInHours / 24);
    if (diffInDays < 7) return t('notifications.daysAgo', '{{count}}d ago').replace('{{count}}', diffInDays);
            ...
            <div className="max-h-64 overflow-y-auto">
              {loading ? (
                ...
                <div className="p-6 text-center">
                  ...
                  <p className="text-sm text-gray-500 font-medium">{t('notifications.empty', 'No notifications')}</p>
                  <p className="text-xs text-gray-400 mt-1">{t('notifications.willNotify', 'You will be notified when there is something new')}</p>
                </div>
              ) : (
