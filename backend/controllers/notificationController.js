...
exports.getNotificationStream = async (req, res) => {
  try {
    ...
    // Disable compression për këtë response
    ...
    // Flush headers menjëherë
    res.flushHeaders();

    // Send initial connection message
    const connectionMessage = `data: ${JSON.stringify({ type: 'connection', message: 'Connected to notification stream', userId })}\n\n`;
    res.write(connectionMessage);
    console.log(`[DEBUG] Sent connection message to user ${userId}`);
    ...
    console.log(`[DEBUG] EventSource stream established for user ${userId}. Total active streams: ${global.notificationStreams.size}`);

    // Handle client disconnect
    req.on('close', () => {
      console.log(`[DEBUG] Client ${userId} disconnected from notification stream`);
      if (global.notificationStreams && global.notificationStreams.has(userId)) {
        global.notificationStreams.delete(userId);
        console.log(`[DEBUG] Removed stream for user ${userId}. Remaining streams: ${global.notificationStreams.size}`);
      }
    });

    // Handle client error
    req.on('error', (error) => {
      console.error(`[ERROR] EventSource error for user ${userId}:`, error);
      if (global.notificationStreams && global.notificationStreams.has(userId)) {
        global.notificationStreams.delete(userId);
      }
    });
    ...
    const heartbeat = setInterval(() => {
      if (global.notificationStreams && global.notificationStreams.has(userId)) {
        try {
          ...
          console.log(`[DEBUG] Sent heartbeat to user ${userId}`);
        } catch (error) {
          console.error(`[ERROR] Failed to send heartbeat to user ${userId}:`, error);
        ...
        console.log(`[DEBUG] User ${userId} no longer has active stream, stopping heartbeat`);
        clearInterval(heartbeat);
      }
    ...
    console.error('[ERROR] Error setting up notification stream:', error);
